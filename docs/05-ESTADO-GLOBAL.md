...
| **Branch / git** | ⚙️ **NO se escribe a mano — lo GENERA el heartbeat en cada boot** → `docs/.estado-auto.md` (rama, HEAD, sucios, deuda de consolidación). Un dato volátil copiado aquí se desincroniza siempre: esta fila mintió 42 días (N2-01). Lo estable: se trabaja en **rama** y **el dueño mergea** (CLAUDE.md §Reglas git). La versión del kernel la dice su stamp `scripts/.kernel-version.json` — copiarla aquí sería repetir el error que esta misma celda describe. |
