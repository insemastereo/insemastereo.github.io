...
> **⚠️ VENDORIZADA 2026-09-03 (C4-5 · DICTAMEN-C4 §9 D-C4-22)** del repo *marketingskills* de Corey Haines
> (licencia MIT — ver LICENSE-marketingskills.txt; ruta `skills/attribution`, skill v1.1.0, tag del repo
> **v2.11.0**, consultado el 2026-09-03). **Gobernada nº 44**; entra porque ataca el techo YA DOCUMENTADO de
> la pauta: la venta se cierra por WhatsApp, FUERA del píxel. Cross-refs del origen → nuestros nombres:
> ads=paid-ads · ab-testing=ab-test-setup. **NO tenemos** `analytics` (lo más cercano: `ga4-lead-tracking`),
> ni `revops` (lo más cercano: `crm-architect`), ni `ai-seo` (lo más cercano: `semantic-schema-aeo`) — donde el
> texto los cite, salta a esos. Ignorar los links a `../../tools/` (no vendorizado; sus CLIs usan APIs MUERTAS).
> **NO se vendorizó** `evals/evals.json` del origen (igual que en `offers`): no lo referencia el cuerpo.
> **Capa ALTORRA obligatoria**: español al hablar · compliance = COLOMBIA (Ley 1581/2012, SIC; NO
> portar CAN-SPAM/TCPA/GDPR como si aplicaran) · gate `legal-colombia` antes de prometer nada · **cero cifras
> inventadas** (guardarraíles de `marketing-psicologico-conversion` §10). Reglas propias → § Puente ALTORRA.
...
|---|---|---|
| **Ad platforms** (Google/Meta/LinkedIn) | Over-counts *itself* | Claims view-through + click conversions in its own window; every platform claims credit for the same sale; motivated to look good |
...
## Puente ALTORRA — reglas propias, con Bersaglio como caso medido (C4-5 · D-C4-22, 2026-09-03)

Todo lo de arriba es el texto del origen, pensado para SaaS y DTC de Estados Unidos. Esto es lo que
cambia aquí, y no es cosmética: **en esta cartera la venta se cierra por WhatsApp, fuera del píxel**.

1. **El techo de esta cartera tiene nombre y ya está medido: la conversión ocurre FUERA del píxel.**
   La pauta lleva a una conversación de WhatsApp; la venta se cierra ahí, a veces días después, a
   veces en persona. Meta solo ve «conversación iniciada», así que **optimiza hacia quien chatea, no
   hacia quien compra** — y ninguna elección de modelo de atribución arregla eso, porque el dato de
   compra nunca entró. Antes de discutir first-touch vs last-touch con el dueño, di esta frase: el
   problema no es el reparto del crédito, es que **falta la señal**. *(procedencia: `meta-ads-diagnostico`
   D-5 — «no cambies el objetivo: cambia la SEÑAL»; y el techo escrito en `docs/44-PAUTA-META.md` de
   Bersaglio.)*
2. **Aquí la «self-reported attribution» del §4 no es el plan B: es el plan A.** Sin píxel en el
   cierre y con ciclos largos (un inmueble, una joya de varios millones), la pregunta *«¿cómo supo de
   nosotros?»* hecha **en la primera respuesta de WhatsApp** captura lo que ninguna herramienta ve. Se
   registra como campo del CRM (→ `crm-architect`), no en la cabeza del vendedor. Y se lee con la
   advertencia del origen: la gente acredita el toque MEMORABLE, no el primero.
   *(procedencia: DICTAMEN-C4 §9 D-C4-22.)*
3. **La fuente de verdad es el CRM/el cierre real, jamás el panel de Meta — y NUNCA se suman
   plataformas.** La regla 2 del §5 del origen se aplica tal cual y sin excepción: si Meta reclama 40
   conversaciones y Google 15 pero hubo 12 cierres, hay **12**, con reclamantes solapados. Cualquier
   informe que sume paneles es ficción. *(procedencia: origen §5 regla 2 + `paid-ads`
   `references/audit-guardrails.md` → «Hard stops».)*
4. **Cero cifras inventadas, y el gate `legal-colombia` antes de publicar cualquier número.** Un
   readout de atribución produce porcentajes y pesos: si un número no sale de un dato leído este turno
   (panel, CRM, exportación), va marcado como estimación con su base, o no va. Nada de «tu ROAS
   real es 3,2x» sin decir de dónde salió, y nada de prometer resultados en pauta.
   *(procedencia: `marketing-psicologico-conversion` §10 + §3.3 del router: evidencia antes de afirmar.)*
5. **La ley colombiana manda sobre el Pilar B.** El «identity graph» del origen une navegación anónima
   con una persona identificada: eso es tratamiento de datos personales bajo la **Ley 1581/2012**
   (autorización previa, finalidad declarada, derecho de supresión). Antes de instrumentar cualquier
   `identify()`, stitching cross-dominio o volcado al CRM: pasa por `legal-colombia` y por la política
   de privacidad publicada. **Jamás** se mete un correo o un teléfono en una URL de terceros — el
   «anonymity guard, fail closed» del origen no es una buena práctica aquí, es cumplimiento.
   *(procedencia: DICTAMEN-C4 §9 D-C4-22 + `legal-colombia`.)*
6. **MMM y las cohortes de 2–3 años NO aplican todavía; el experimento de encendido/apagado sí.** Con
   el presupuesto de esta cartera, la única pieza de incrementalidad ejecutable hoy es el **on/off**
   sobre una campaña concreta, midiendo conversaciones Y cierres. Recomendar un MMM aquí es vender
   maquinaria para un dato que no existe. *(procedencia: origen §3 «scale to the user» + el criterio
   de coste del router: $0/free-tier antes que herramienta nueva.)*
