...
> Repo: `github.com/coreyhaines31/marketingskills` · ruta: `skills/ads/references/payback-period.md`
...
> Cross-refs del origen → nuestros nombres: `ads` = `paid-ads` · `attribution` = `attribution` (nuestra nº 44) · `pricing` NO está vendorizada.
> La capa ALTORRA NO vive aquí: vive en el `SKILL.md`. Ojo: la aritmética es de SaaS por suscripción — en inmobiliaria/joyería el ARPU mensual no existe tal cual; se adapta, no se copia.
